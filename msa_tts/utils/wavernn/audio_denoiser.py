...
class AudioDenoiser():
    def __init__(self, noise_profile_path):
        self.noise_clip, sr_noise = librosa.load(noise_profile_path)
        # self.noise_clip, sr_noise = np.load(noise_profile_path, allow_pickle=True)  
        self.noise_len = len(self.noise_clip/float(sr_noise))
    
    def denoise(self, wav):
        denoised_wav = reduce_noise(wav,
                                    self.noise_clip,
                                    n_grad_freq=4,
                                    n_grad_time=0,
                                    n_fft=1024,
