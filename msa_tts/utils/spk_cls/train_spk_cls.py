...
def train_spk_cls(params):
    ...
    # ========== Target speakers
    target_speakers_main = params["dataset_train"]["speakers_list"]
    print(target_speakers_main)
    random.Random(int(params["spk_seed"])).shuffle(target_speakers_main)
    print("Target speakers in order:")
    print(target_speakers_main)
    
    
    for itr_order in range(1, len(target_speakers_main)+1):
        print(f"=========== Iteration {itr_order}")

        target_speakers = target_speakers_main[:itr_order]
        spk_to_id = {spk:itr for (itr, spk) in enumerate(target_speakers[:itr_order])}
        ...
        def train():
            ...
            for epoch in range(n_epochs):
                ...
                test_accuracy = test()
                print(f"Epoch {epoch}/{n_epochs} loss: {loss_epoch}, test acc: {test_accuracy}")
        ...
        checkpoint_path = os.path.join(params["out_path"], f"ckpt_{itr_order-1}.pt")
        
        torch.save(model.state_dict(), checkpoint_path)

        spk2id_path = os.path.join(params["out_path"], f"spk2id_{itr_order-1}.yml")
